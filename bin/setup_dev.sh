...
oc set probe dc/tasks --readiness --get-url=http://:8080/ --initial-delay-seconds=60 --timeout-seconds=10 -n ${GUID}-tasks-dev
oc set probe dc/tasks --liveness --get-url=http://:8080/ --initial-delay-seconds=60 --timeout-seconds=10 -n ${GUID}-tasks-dev
oc set resources dc/tasks --limits=memory=2Gi,cpu=2 --requests=memory=2Gi,cpu=1 -n ${GUID}-tasks-prod
