...
oc set probe dc/tasks-blue --readiness --get-url=http://:8080/ --initial-delay-seconds=60 --timeout-seconds=30 -n ${GUID}-tasks-prod
oc set probe dc/tasks-blue --liveness --get-url=http://:8080/ --initial-delay-seconds=60 --timeout-seconds=30 -n ${GUID}-tasks-prod
...
oc set probe dc/tasks-green --readiness --get-url=http://:8080/ --initial-delay-seconds=60 --timeout-seconds=30 -n ${GUID}-tasks-prod
oc set probe dc/tasks-green --liveness --get-url=http://:8080/ --initial-delay-seconds=60 --timeout-seconds=30 -n ${GUID}-tasks-prod
oc set resources dc/tasks-blue --limits=memory=2Gi,cpu=2 --requests=memory=1Gi,cpu=1 -n ${GUID}-tasks-prod
# Setting 'wrong' VERSION. This will need to be updated in the pipeline
